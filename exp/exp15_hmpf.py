...
train_df = transaction_df.loc[(transaction_df.t_dat >= init_date) & (transaction_df.t_dat <= last_date)].copy()        .merge(group_df, on="customer_id", how="left")        .groupby(["group", "article_id"]).t_dat.count().reset_index()
...
sub_df["prediction"] = sub_df[["last_purchase", "other_colors", "popular_items", "age_items"]]                 .progress_apply(blend, w=[100, 10, 1, 1], axis=1, k=32).apply(prune, ok_set=sold_set)
